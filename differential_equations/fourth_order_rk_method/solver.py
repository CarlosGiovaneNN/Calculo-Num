...
def run_fourth_order_rk():
    ...
    fourth_order_rk(f, x0, y0, h, n, f_analytical)
    return


def fourth_order_rk(f, x0, y0, h, n, f_analytical=None):
    x = x0
    y = y0
    ...
    if f_analytical:
        ...
        print(f"\n{'Step':<5} | {'x':<8} | {'y_RK4':<10} | {'y_Exata':<10} | {'Erro':<10}")
        print("-" * 55)
    ...
    for i in range(n):
        ...
        if f_analytical:
            ...
            y_exact_vals.append(y_exact)
            errors.append(error)
            print(f"{i+1:<5} | {x:<8.4f} | {y:<10.4f} | {y_exact:<10.4f} | {error:<10.6f}")
        else:
    ...
    if f_analytical:
        ...
        plt.subplot(1, 2, 1)
        plt.plot(x_vals, y_num_vals, 'o--', label='RK4 (Numerical)')
        plt.plot(x_vals, y_exact_vals, 'r-', label='Analytical (Exact)', alpha=0.7)
        plt.title(f"Solution: RK4 (h={h})")
        plt.xlabel("x")
        plt.ylabel("y")
        plt.legend()
        plt.grid(True)
        ...
        plt.subplot(1, 2, 2)
        plt.plot(x_vals, errors, 'k-o', color='purple')
